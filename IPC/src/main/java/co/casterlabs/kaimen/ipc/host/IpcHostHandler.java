package co.casterlabs.kaimen.ipc.host;
...
import co.casterlabs.kaimen.ipc.client.IpcClientEntryPoint;
...
public abstract class IpcHostHandler implements Closeable {
    ...
    public IpcHostHandler(@NonNull Class<? extends IpcClientHandler> handlerClass) throws IOException {
        try {
        ...
        List<String> exec = getExec(IpcClientEntryPoint.class, handlerClass.getTypeName());
        ...
        AsyncTask.createNonDaemon(() -> {
            try {
                while (true) {
                    ...
                    // Send a ping.
                    this.connection.sendMessage(PING_PACKET);
                }
            } catch (Exception e) {
                showMessage(StringUtil.getExceptionStack(e));
                return;
        ...
        protected void handleMessage(Object message) {
            if (message instanceof KaimenIpcPacket) {
                ...
                switch (packet.getType()) {
                    ...
                    case PRINT: {
                        KaimenIpcPrintPacket printPacket = (KaimenIpcPrintPacket) packet;
                ...
                dispatchThread.submitTask(() -> {
                    // Different method.
                    IpcHostHandler.this.handleMessage(message);
                });
            }
        }
